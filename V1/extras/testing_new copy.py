...
def generate_ai_email(user_name, previous_responses):
    if not GOOGLE_API_KEY:
        ...
        return {
            "sender": f"training-{random.randint(1, 1000)}@cybervantage.com",
            "subject": f"Security Training Email #{random.randint(1, 100)}",
    ...
    try:
        ...
        # Map standard category names to their enum values if they exist
        category_mappings = [
        ...
        for long_name, short_name in category_mappings:
            if long_name in harm_categories:
                safety_settings[getattr(HarmCategory, long_name)] = HarmBlockThreshold.MEDIUM_AND_ABOVE
            elif short_name in harm_categories:
                safety_settings[getattr(HarmCategory, short_name)] = HarmBlockThreshold.MEDIUM_AND_ABOVE
    
    except Exception as e:
        print(f"Warning: Could not set up safety settings: {e}")
        safety_settings = {}  # Empty dict is safer than potentially incorrect settings
    
    try:
        # Generate the email - try with and without safety settings
        response = None
        try:
            if safety_settings:
            ...
            print(f"Error with model generation: {model_error}")
            raise
        
        # Extract text from response using multiple approaches
        content = None
        
        # Try different ways to access the response content
        if hasattr(response, 'text'):
            content = response.text
            print("Accessed response via .text property")
        elif hasattr(response, 'parts') and response.parts:
            content = response.parts[0].text
            print("Accessed response via .parts[0].text")
            ...
            if hasattr(response.candidates[0].content, 'parts') and response.candidates[0].content.parts:
                ...
                print("Accessed response via .candidates[0].content.parts[0].text")
        
        # If we still don't have content, try to stringify the whole response
        if not content:
            content = str(response)
            print(f"Falling back to string representation of response: {content[:100]}...")
        
        # Check if we have content to work with
        if not content or len(content) < 20:  # Arbitrary minimum length
            raise ValueError(f"Received insufficient content from API: {content}")
        ...
        try:
            ...
            sender_match = content.split("Sender: ", 1)
            if len(sender_match) > 1:
                sender = sender_match[1].split("\n", 1)[0].strip()
            else:
                sender = f"training-{random.randint(1, 1000)}@cybervantage.com"
            
            subject_match = content.split("Subject: ", 1)
            if len(subject_match) > 1:
                subject = subject_match[1].split("\n", 1)[0].strip()
            else:
                subject = f"Training Email #{random.randint(1, 100)}"
            
            date_match = content.split("Date: ", 1)
            if len(date_match) > 1:
                date = date_match[1].split("\n", 1)[0].strip()
            else:
                date = datetime.datetime.now().strftime("%B %d, %Y")
            
            content_match = content.split("Content: ", 1)
            if len(content_match) > 1 and "Is_spam:" in content_match[1]:
                email_content = content_match[1].split("Is_spam:", 1)[0].strip()
            else:
                # If we can't extract content properly, use the whole response as content
                email_content = f"<p>Training email content:</p><pre>{content}</pre>"
            
            is_spam_match = content.split("Is_spam: ", 1)
            if len(is_spam_match) > 1:
                is_spam = "true" in is_spam_match[1].lower()
            else:
                is_spam = random.choice([True, False])
            
            # Format HTML content properly if it's not already
            if not email_content.strip().startswith("<"):
                # Convert plain text to HTML
                lines = email_content.strip().split("\n")
                email_content = "".join([f"<p>{line}</p>" for line in lines if line.strip()])
            ...
            # Return a simplified email with the raw content for debugging
            is_spam = random.choice([True, False])
            return {
                "sender": f"debug-{random.randint(1, 1000)}@cybervantage.com",
                "subject": f"Debug Email #{random.randint(1, 100)}",
                "date": datetime.datetime.now().strftime("%B %d, %Y"),
                "content": f"<p>Parsing error occurred. Raw content:</p><pre>{content[:500]}</pre>",
                "is_spam": is_spam
            }
        ...
        if "429" in error_str or "quota" in error_str.lower() or "rate limit" in error_str.lower():
            ...
            # Return a rate limit specific message
            return {
                "sender": "system@cybervantage.com",
                "subject": "API Rate Limit Notification",
                "date": datetime.datetime.now().strftime("%B %d, %Y"),
                "content": """
                    <p>This is a system notification.</p>
                    <p>The API rate limit has been reached. Please try again in a few minutes.</p>
                    <p>In the meantime, you can continue with this training email.</p>
                """,
                "is_spam": False
            }
        
        # For other errors, provide a realistic training email
        phishing_templates = [
            {
                "sender": "security-alerts@bankofamerica-secure.net",
                "subject": "Urgent: Your Account Has Been Limited",
                "content": """
                    <p>Dear Valued Customer,</p>
                    <p>We have detected unusual sign-in activity on your Bank of America account. For your protection, we have temporarily limited some features.</p>
                    <p>To remove these limitations, please verify your account by clicking on the link below:</p>
                    <p><a href="https://secure-bankofamerica.com/verify.html">Verify Account Now</a></p>
                    <p>If you do not verify your account within 24 hours, your account access may be permanently disabled.</p>
                    <p>Bank of America Security Team</p>
                """,
                "is_spam": True
            },
            {
                "sender": "no-reply@amazonsupport.co",
                "subject": "Your Amazon Order Has Been Placed",
                "content": """
                    <p>Hello,</p>
                    <p>We're contacting you about order #38947592 for Apple MacBook Pro ($1,899.99) that was recently placed from your account.</p>
                    <p>If you did not make this purchase, please contact us immediately by calling our customer service at 1-888-280-5674 or clicking the link below:</p>
                    <p><a href="http://amazon-order-support.co/dispute">Dispute This Charge</a></p>
        ...
        legitimate_templates = [
            {
                ...
                "subject": "Your Morning Briefing: Top Stories for Today",
                "content": """
                    <p>Good morning,</p>
                    <p>Here are today's top stories:</p>
                    <ul>
                        <li>Global markets react to new economic data</li>
                        <li>Scientists announce breakthrough in renewable energy storage</li>
                        <li>Tech industry prepares for annual developer conference</li>
                        <li>Sports update: Championship series continues tonight</li>
                    </ul>
                    <p>Read more at <a href="https://www.nytimes.com">nytimes.com</a></p>
                    <p>To manage your email preferences or unsubscribe, <a href="https://www.nytimes.com/email">click here</a>.</p>
                """,
                "is_spam": False
            },
            {
                "sender": "support@dropbox.com",
                "subject": "Your storage space is almost full",
                "content": """
                    <p>Hi there,</p>
                    <p>Your Dropbox is almost full! You've used 98% of your storage space.</p>
                    <p>When you run out of space, you won't be able to save new files to Dropbox.</p>
                    <p>Options to get more space:</p>
                    <ul>
                        <li>Upgrade to Dropbox Professional</li>
                        <li>Refer friends to earn more space</li>
                        <li>Clean up unused files</li>
                    </ul>
                    <p>Visit your <a href="https://www.dropbox.com/account">account settings</a> to manage your storage.</p>
        ...
        # Choose random template based on coin flip
